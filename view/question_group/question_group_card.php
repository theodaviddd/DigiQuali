...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/digiquali/view/question_group/question_group_list.php', 1);

	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) $backtopage = $backurlforlist;
			else $backtopage = dol_buildpath('/digiquali/view/question_group/question_group_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
...
if ($action == 'create') {
	...
	if (!empty($conf->categorie->enabled)) {
		...
		$categoryArborescence = $form->select_all_categories('question_group', '', 'parent', 64, 0, 1);
        ...
        print '<a class="butActionNew" href="' . DOL_URL_ROOT . '/categories/index.php?type=question_group&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?action=create') . '" target="_blank"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans('AddCategories') . '"></span></a>';
		print "</td></tr>";
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($conf->categorie->enabled) {
		...
		print $form->showCategories($object->id, 'question_group', 1);
		print "</td></tr>";
	}
